...
class ResUnet_trio(torch.nn.Module):
    def __init__(self, nc_a, nc_b, cfg, init_func, init_to_identity):
        super(ResUnet_trio, self).__init__()
        act = down_activation[cfg]
        # ------------ Down-sampling path
        self.ndown_blocks = len(ndf[cfg])
        self.nup_blocks = len(nuf[cfg])
    ...
    def forward(self, img_a, img_b, img_c):
        x = torch.cat([img_a, img_b, img_c], 1)
        skip_vals = {}
        conv_num = 1
        # Down
        while conv_num <= self.ndown_blocks:
...
class Reg_trio(nn.Module):
    def __init__(self,height,width,in_channels_a,in_channels_b):
        super(Reg_trio, self).__init__()
       #height,width=256,256
        #in_channels_a,in_channels_b=1,1
        init_func = 'kaiming'
        init_to_identity = True
        ...
        self.in_channels_b = in_channels_b
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.offset_map = ResUnet_trio(self.in_channels_a, self.in_channels_b, cfg='A', init_func=init_func, init_to_identity=init_to_identity).to(
            self.device)
        self.identity_grid = self.get_identity_grid()
    ...
    def forward(self, img_a, img_b, img_c, apply_on=None):

        deformations = self.offset_map(img_a, img_b, img_c)
